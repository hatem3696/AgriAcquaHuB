...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (
        empty($Type) && empty($Name) && empty($Price) && empty($Days) && empty($P_left)
    ) {
        ...
        $_SESSION['updatePackError'] = "Duration cannot be empty";
        header("Location: ../../View/Admin/UpdatePackage.php?updatePack=$Package_Id");
        exit();
        ...
        !empty($Name) && !empty($Type) && !empty($Price) && !empty($Days) && !empty($P_left) && !empty($Start_Date)
    ) {
        $isValid = true;
    }
    ...
    if (
        !empty($Name) && !empty($Type) && !is_numeric($Name) && !empty($Price) && !empty($P_left) && !empty($Days) && is_numeric($Price)
        && is_numeric($P_left) && is_numeric($Days) && !empty($Start_Date) && $isValid === true && $pkgNameValidity = true
    ) {
        $result = updatePackage($Package_Id, $Name, $Type, $Description, $Price, $Days, $P_left, $Start_Date, $End_Date, $Image_url);
        if ($result) {
